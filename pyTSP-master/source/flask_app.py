...
def index():
    ...
    top10_cities = City.query.order_by(City.population.desc()).limit(10).all()
    top10 = [c.name for c in top10_cities]
...
def import_cities():
    """Chỉ import 10 thành phố lớn nhất từ data/vn_cities.json vào DB."""
    vn_path = join(path_parent, 'data', 'vn_cities.json')
    ...
    # Lấy 10 thành phố có population cao nhất
    top10_list = sorted(
        all_cities,
        key=lambda d: int(d.get('population') or 0),
        reverse=True
    )[:10]

    # Lưu vào DB
    for idx, d in enumerate(top10_list, start=1):
        city = City(
            id         = idx,
            name       = d['name'],
...
def create_app():
    # template_folder trỏ vào source/templates
    app = Flask(__name__,
